...
let DH = window.DH = {
    ...
    * */
     ...
     *  type: 0-6(周日-周六)
     * */
    getSunday: function(dataStr,type){
        var date = DH.parseDate(dataStr);
        var state = false;
        if(date.getDay() === type){
            state = true;
        }
    ...
    * */
    ...
    //获取时间差-详情页使用
    getTimestamp:function(statrT,endT){
        var zdate = DH.parseDate(statrT);
        var edate = DH.parseDate(endT);
        var time = zdate - edate;
...
String.prototype.formatDate=function(format){
    ...
    format =  window.LANGUAGES === 'En' ? 'yyyy/MM/dd' : format;
